...
public class MonomeConfiguration extends OSCDeviceConfiguration<Page> {
	...
	// varibright
	public void vari_led(int x, int y, int value, int index) {
		if (x < 0 || y < 0 || value < 0 || x >= this.sizeX || y >= this.sizeY || value > 15) {
			return;
		}
		...
		try {
			if (this.serialOSCPort == 0) {
				...
				// varibright
				msg = new OSCMessage(this.prefix + "/grid/led/level/set", args);
				serialOSCPortOut.send(msg);
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	public void vari_led_col(ArrayList<Integer> intArgs, int index) {
		int col = intArgs.get(0);
		if (col < 0 || col >= sizeX) {
			return;
		}
		...
		for (int i = 1; i < intArgs.size(); i++) {
			...
			values[i-1] = intArgs.get(i);
			this.vari_led(col, i-1, values[i-1], index);
			numValues++;
		}
	}
	
	// broken (umut), using a for loor of regular instead
	// format of the arraylist argument, column number then value 0-15 for each column
//	public void vari_led_col(ArrayList<Integer> intArgs, int index) {
//		int col = intArgs.get(0);
//		if (col < 0 || col >= sizeX) {
//			return;
...
//		
//		if (index > -1) {
//			for (int y=0; y < intArgs.size() ; y++) {
//				if (y >= sizeY) {
//					break;
//				}
//				this.pageState[index][col][y] = values[y];
//			}
//	
//			if (index != this.curPage) {
//				return;
...
//			}
//
//			for (int y=0; y < (intArgs.size() - 1); y++) {
//				this.ledState[col][y] = values[y];			
//			}
//			
//			if (this.deviceFrame != null) {
//				MonomeDisplayFrame monomeDisplayFrame = monomeFrame.getMonomeDisplayFrame();
...
//				newArgs[0] = intArgs.get(0);
//				int iii;
//				for(iii=1;iii<intArgs.size();iii++) {
//					newArgs[iii] = intArgs.get(iii);
//				}
////				for(iii=intArgs.size();iii<=this.sizeY;iii++)
////					newArgs[iii] = new Integer(0);
////				}
//				msg = new OSCMessage(this.prefix + "/grid/led/levels/col", newArgs);
//				serialOSCPortOut.send(msg);
//			}
//		} catch (Exception e) {
//			e.printStackTrace();
...
//	}
	...
	// not tested (umut)
	public void vari_led_map(ArrayList<Integer> intArgs, int index) {
		int xOffset = intArgs.get(0);
		int yOffset = intArgs.get(1);
		for (int y = 0; y < 8; y++) {
			for (int x = 0 ; x < 8; x++) {
				int val = intArgs.get(8*y + x +2);
				this.pageState[index][x + xOffset][y + yOffset] = val;
				if (index != this.curPage) {
					continue;
				}
				this.ledState[x + xOffset][y + yOffset] = val;
			}
		}
		if (index == this.curPage) {
			if (this.deviceFrame != null) {
		...
		try {
			...
			msg = new OSCMessage(this.prefix + "/grid/led/levels/map", args);
			if (serialOSCPortOut != null) {
				serialOSCPortOut.send(msg);
			}
		} catch (Exception e) {
