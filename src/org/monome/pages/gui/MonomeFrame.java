...
public class MonomeFrame extends JInternalFrame implements Serializable, DeviceFrame {
	...
	private JMenuItem syncTimingItem = null;
	...
	private JMenu getConfigurationMenu() {
		if (configurationMenu == null) {
			...
			configurationMenu.addSeparator();			
			configurationMenu.add(getSyncTimingItem());
			configurationMenu.addSeparator();
	...
	private JMenuItem getSyncTimingItem() {
		if (syncTimingItem == null) {
			syncTimingItem = new JMenuItem();
			syncTimingItem.setText("Sync Timing");
			syncTimingItem.setMnemonic(KeyEvent.VK_T);
			syncTimingItem.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					MonomeConfiguration monomeConfig = MonomeConfigurationFactory.getMonomeConfiguration(index);
					if (monomeConfig.pages.size() > 0) {
						for(int i=0;i < monomeConfig.pages.size(); i++) {
							monomeConfig.pages.get(i).handleReset();
						}
					}
				}
			});
		}
		return syncTimingItem;
	}
